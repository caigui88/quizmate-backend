package com.quizmate.gateway.config.swagger;
...
class SwaggerDocRegister extends Subscriber<InstancesChangeEvent> {
    private final static String[] EXCLUDE_ROUTES = new String[]{"quizmate-gateway", "quizmate-auth", "quizmate-file", "quizmate-monitor"};
    @Autowired
    private SwaggerUiConfigProperties swaggerUiConfigProperties;
    @Autowired
    private DiscoveryClient discoveryClient;
    ...
    public void onEvent(InstancesChangeEvent event) {
        Set<AbstractSwaggerUiConfigProperties.SwaggerUrl> swaggerUrlSet = discoveryClient.getServices().stream().flatMap(serviceId -> discoveryClient.getInstances(serviceId).stream()).filter(instance -> !StringUtils.equalsAnyIgnoreCase(instance.getServiceId(), EXCLUDE_ROUTES)).map(instance -> {
